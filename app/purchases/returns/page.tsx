...
export default async function SupplierReturns() {
  const company = await getCompany();
  if (!company) return <div className="empty">No company found.</div>;

  const returns = (await getDocuments(company.id, "PURCHASE_RETURN")) as any[];
  ...
  return (
    <>
      ...
      <div className="actions">
        <Link href="/purchases/returns/new" className="btn">+ New Return</Link>
      </div>
      ...
      <section>
        <div className="card">
          ...
          <DataTable
            rows={rows}
            emptyLabel="No supplier returns"
